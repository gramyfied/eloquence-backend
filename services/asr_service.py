...
class AsrService:
    ...
    def _transcribe_sync(self, audio_float32: np.ndarray, language: str) -> str:
        ...
        if self.model is None:
            ...
            logger.error(f"ASR_ERROR: {error_msg}")
            raise RuntimeError(error_msg)

        try:
            # Transcrire l'audio (numpy array float32)
            # beam_size=5 est une valeur par défaut courante
            logger.debug(f"Appel de self.model.transcribe avec audio_float32 shape={audio_float32.shape}, dtype={audio_float32.dtype}, language={language}")

            segments, info = self.model.transcribe(audio_float32, language=language, beam_size=5)

            logger.debug(f"Transcription terminée. Langue détectée: {info.language} avec probabilité {info.language_probability:.2f}")
            ...
            logger.error(f"ASR_ERROR: {error_msg}", exc_info=True)
    ...
    async def transcribe(self, audio_bytes: bytes, language: str) -> str:
        ...
        try:
            ...
            logger.debug(f"Type des données audio: {type(audio_bytes)}")
            logger.debug(f"Premiers octets: {audio_bytes[:20] if len(audio_bytes) > 20 else audio_bytes}")
            ...
            try:
                ...
                logger.debug(f"Audio lu par soundfile: shape={audio_data.shape}, dtype={audio_data.dtype}, sample_rate={sample_rate}")
